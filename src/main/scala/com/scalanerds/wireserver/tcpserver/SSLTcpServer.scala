...
/***
  ...
  */
class SSLTcpServer(props: (InetSocketAddress, InetSocketAddress) => Props,
                   address: String,
                   port: Int)
  ...
  override def handler: Sink[Tcp.IncomingConnection, Future[Done]] =
    Sink.foreach[Tcp.IncomingConnection] { conn =>
      ...
      val in: Sink[ByteString, NotUsed] =
        Flow[ByteString].to(Sink.actorRef(actor, GracefulKill))

      val out: Source[ByteString, Unit] = Source
      ...
      conn handleWith serverSSL.reversed
        .join(ssl)
        .alsoTo(Sink.onComplete(_ => logger.debug("Client disconnected")))
    }
}
