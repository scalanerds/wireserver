...
object PlainTcpServer {
  def props(props: Props, address: String = "localhost", port: Int = 3000): Props =
    Props(classOf[PlainTcpServer], props, address, port)
}

class PlainTcpServer(props: Props, address: String, port: Int) extends TcpServer(address, port) {
