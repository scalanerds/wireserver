...
/** *
  ...
  */
class DualTcpServer(props: (InetSocketAddress, InetSocketAddress) => Props,
                    address: String,
                    port: Int)
    extends TcpServer(address, port)
    with TcpSSL
    with TcpFraming {
  ...
  override def handler: Sink[Tcp.IncomingConnection, Future[Done]] =
    Sink.foreach[Tcp.IncomingConnection] { conn =>
      ...
      val streamHandler: ActorRef =
        context.actorOf(props(conn.remoteAddress, conn.localAddress))

      val getFlow = (name: String) => {
        val forwarderActor: ActorRef =
          context.actorOf(Props(new ForwarderActor(streamHandler)),
                          name + Math.random())
        val in: Sink[ByteString, NotUsed] =
          Flow[ByteString].to(Sink.actorRef(forwarderActor, GracefulKill))
        val out = Source
          .actorRef[ByteString](100, OverflowStrategy.fail)
      ...
      val sslFlow: Flow[ByteString, ByteString, NotUsed] = serverSSL.reversed
        .join(ssl)
      ...
      val router = Flow.fromGraph(GraphDSL.create() { implicit b =>
        ...
        val src = b.add(Flow[ByteString].map(stream => {
          ...
          if (isSSL.isEmpty)
            isSSL = Some(stream(0) == 22 && stream.length != 22)
          stream
        }))
...
object DualTcpServer {
  ...
  def props(props: (InetSocketAddress, InetSocketAddress) => Props,
